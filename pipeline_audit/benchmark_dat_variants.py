#!/usr/bin/env python3
"""Benchmark DAT-light, DAT-S, DAT-2, HAT-S x4 — speed + PieAPP."""
...
def bench_model(input_path, output_path, model_path, gt_path, name, n_frames=60, dtype_str='fp16'):
    ...
    print(f"  Loading {name} ({dtype_str})...", flush=True)
    ...
    print(f"  {nparams:.2f}M params, pad={need_pad}", flush=True)

    # Warmup
    dummy = torch.randn(1, 3, h + pad_h, w + pad_w, device='cuda', dtype=dtype)
    try:
        with torch.no_grad():
            _ = model(dummy)
            torch.cuda.synchronize()
    except Exception as e:
        print(f"  Warmup FAILED ({dtype_str}): {e}", flush=True)
        if dtype_str == 'fp16':
            print(f"  Retrying with bf16...", flush=True)
            model = ModelLoader(device="cuda:0").load_from_file(str(model_path)).model.eval().bfloat16().cuda()
            dtype = torch.bfloat16; dtype_str = 'bf16'
            dummy = torch.randn(1, 3, h + pad_h, w + pad_w, device='cuda', dtype=dtype)
            with torch.no_grad():
                _ = model(dummy)
                torch.cuda.synchronize()
    ...
    fps_val = total / t_inf if t_inf > 0 else 0
    ext300 = 300 / fps_val if fps_val > 0 else 99999

    print(f"  {total}f in {t_inf:.1f}s ({fps_val:.1f} FPS), VRAM={vram:.0f}MB", flush=True)
    print(f"  Extrapolated 300f: {ext300:.0f}s", flush=True)

    # PieAPP
    print(f"  PieAPP...", flush=True)
    pieapp = measure_pieapp(gt_path, output_path, n_samples=min(16, total))
    ...
    return {
        'model': name, 'dtype': dtype_str, 'params_m': round(nparams, 2),
        'n_frames': total, 'fps': round(fps_val, 2),
        'inference_s': round(t_inf, 2), 'extrapolated_300f_s': round(ext300, 1),
...
def main():
    ...
    models = [
        (f'{S}/DAT_light_x4.pth',      'DAT-light x4', 'fp16'),
        (f'{S}/HAT-S_SRx4.pth',        'HAT-S x4',     'fp16'),
        (f'{S}/DAT_S_x4.pth',          'DAT-S x4',     'fp16'),
        (f'{S}/DAT_2_x4.pth',          'DAT-2 x4',     'fp16'),
    ]

    print(f"{'='*70}")
    print(f"DAT/HAT Variant Benchmark ({N} frames, PieAPP on 16 samples)")
    print(f"{'='*70}")

    results = []
    for path, name, dtype_str in models:
        if not os.path.exists(path):
            print(f"\nSKIP {name} — not found at {path}", flush=True)
            continue

        print(f"\n{'─'*70}")
        print(f"  {name} ({os.path.getsize(path)/1e6:.0f}MB, {dtype_str})")
        print(f"{'─'*70}")

        output = f'/root/pipeline_audit/bench_{name.replace(" ", "_").replace("-","_")}.mp4'
        r = bench_model(PAYLOAD, output, path, GT, name, n_frames=N, dtype_str=dtype_str)
        if r:
            results.append(r)
    ...
    for r in results:
        ...
        feasible = "TARGET" if r['fps'] >= 15 else ("CLOSE" if r['fps'] >= 8 else "SLOW")
        print(f"  {r['model']:<20} {r['params_m']:>6.2f}M  PieAPP={r['pieapp_mean']:.5f}  "
              f"S_F={r['s_f']:.4f}  FPS={r['fps']:>6.1f}  300f≈{r['extrapolated_300f_s']:>5.0f}s  "
              f"VRAM={r['vram_mb']:>5.0f}MB  [{tag}] [{feasible}]")

    print(f"\n  Reference:")
    print(f"  PLKSR (prod):  7.40M  PieAPP=0.41020  S_F=0.1319  FPS= 19.2  VRAM=  694MB  [below]")
    print(f"  DAT (full):   14.80M  PieAPP=0.06400  S_F=~0.37   FPS=  2.0  VRAM= 2878MB  [BONUS!]")
    print(f"  Target: PieAPP < 0.092, FPS ≥ 15, S_F > 0.32")

    with open('/root/pipeline_audit/dat_variants_benchmark.json', 'w') as f:
        json.dump({'timestamp': time.strftime('%Y-%m-%dT%H:%M:%S'), 'results': results}, f, indent=2)
    print(f"\nResults saved to dat_variants_benchmark.json")
